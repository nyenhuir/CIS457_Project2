...
gcc -o fulltestB Project2B.c `pkg-config --cflags gtk+-2.0` `pkg-config --libs gtk+-2.0` -lpthread
*/
...
int main( int   argc, char *argv[] ) { 
  ...
  //initializing the use of gtk
    ...
    gtk_window_set_title (GTK_WINDOW (window), "CoronaVirus Communication Center Location B");
...
void *server(void *arg){
  //Variable declaration for all of the socket communication
     ...
     serv_addr.sin_port = htons(6551); //Our port number
